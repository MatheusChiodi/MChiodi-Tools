...
export function CsvJsonConverter() {
  ...
  const [modo, setModo] = useState<"csvToJson" | "jsonToCsv">("csvToJson");
  ...
  const converter = () => {
    try {
      if (modo === "csvToJson") {
        ...
        const convertido = JSON.stringify(dados, null, 2);
        setResultado(convertido);
      } else {
        const arr = JSON.parse(input);
        if (!Array.isArray(arr)) throw new Error();

        const cabecalhos = Object.keys(arr[0] || {});
        const linhas = arr.map((item) =>
          cabecalhos.map((chave) => JSON.stringify(item[chave] ?? "")).join(",")
        );
  ...
  const toggleModo = () => {
    setModo(modo === "csvToJson" ? "jsonToCsv" : "csvToJson");
    setResultado("");
    setErro("");
    setCopied(false);
  };
    ...
    <main className="mx-auto max-w-4xl px-4">
      ...
      >
        <h1 className="text-4xl font-extrabold text-neutral-50 md:text-6xl">
          Conversor <span className="text-blue-400">CSV ↔️ JSON</span>
        </h1>
        <p className="text-base text-neutral-400 md:text-lg">
          Converta facilmente entre CSV e JSON. Simples, rápido e gratuito.
        </p>
        ...
        <div className="flex flex-col gap-6">
          ...
          <textarea
            ...
            placeholder={`Cole seu ${modo === "csvToJson" ? "CSV" : "JSON"} aqui...`}
            rows={8}
          ...
          <div className="flex flex-col justify-center gap-4 sm:flex-row">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-neutral-700 px-6 py-3 text-sm font-medium text-neutral-300 transition hover:bg-neutral-900"
            >
              <Repeat size={18} /> {modo === "csvToJson" ? "CSV → JSON" : "JSON → CSV"}
            </button>
          </div>
