...
export function JsonYamlConverter() {
  ...
  const [modo, setModo] = useState<"jsonToYaml" | "yamlToJson">("jsonToYaml");
  ...
  const converter = () => {
    try {
      if (modo === "jsonToYaml") {
        ...
        const convertido = yaml.dump(obj);
        setResultado(convertido);
      } else {
        const obj = yaml.load(input);
        const convertido = JSON.stringify(obj, null, 2);
        setResultado(convertido);
      }
      setErro("");
    } catch {
  ...
  const toggleModo = () => {
    setModo(modo === "jsonToYaml" ? "yamlToJson" : "jsonToYaml");
    setResultado("");
    setErro("");
    setCopied(false);
  };
    ...
    <main className="mx-auto max-w-4xl px-4">
      ...
      >
        <h1 className="text-4xl font-extrabold text-neutral-50 md:text-6xl">
          Conversor <span className="text-blue-400">JSON ↔️ YAML</span>
        </h1>
        <p className="text-base text-neutral-400 md:text-lg">
          Converta facilmente entre JSON e YAML. Simples, rápido e gratuito.
        </p>
        ...
        <div className="flex flex-col gap-6">
          ...
          <textarea
            ...
            placeholder={`Cole seu ${modo === "jsonToYaml" ? "JSON" : "YAML"} aqui...`}
            rows={8}
          ...
          <div className="flex flex-col justify-center gap-4 sm:flex-row">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-neutral-700 px-6 py-3 text-sm font-medium text-neutral-300 transition hover:bg-neutral-900"
            >
              <Repeat size={18} /> {modo === "jsonToYaml" ? "JSON → YAML" : "YAML → JSON"}
            </button>
          </div>
